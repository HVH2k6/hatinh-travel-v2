...
interface CulturalArtDetailViewProps {
  item: CulturalArt;
}

export function CulturalArtDetailView({ item }: CulturalArtDetailViewProps) {
  const t = useTranslations('attraction.detail'); // fallback for general translations
  const [viewCount, setViewCount] = React.useState(item.views);
    ...
    const incrementView = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_URL}/cultural-arts/${cleanSlug}/view`,
          {
            method: 'POST',
            headers: {
  ...
  const albumImages = React.useMemo(() => {
    ...
    if (item.list_image && Array.isArray(item.list_image)) {
      images.push(...item.list_image);
    }
    return images;
  }, [item.image, item.list_image]);
      ...
      <div className='relative h-[40vh] md:h-[55vh] w-full bg-slate-950'>
        ...
        <div className='absolute top-6 left-4 lg:left-8 z-10'>
          ...
          >
            ...
            <span className='hidden sm:inline'>{t('home') || 'Trở lại'}</span>
          </Link>
        </div>
      ...
      <div className='container mx-auto px-4 lg:px-8 mt-8'>
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 items-start'>
          <div className='lg:col-span-2 space-y-8'>
            ...
            {item.link_video && (
              <div className='bg-white border border-slate-100 rounded-3xl p-5 md:p-8 shadow-sm space-y-4'>
                ...
                <div className='relative w-full aspect-video rounded-2xl overflow-hidden border border-slate-100 bg-slate-50'>
                  {item.link_video.includes('<iframe') ? (
                    <div 
                      className="w-full h-full [&>iframe]:w-full [&>iframe]:h-full"
                      dangerouslySetInnerHTML={{ __html: item.link_video }} 
                    />
                  ) : (
                    <iframe
                      src={item.link_video}
            ...
            <ReviewSection type='cultural-art' id={item.id} />
            ...
            <div className='bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-6'>
              ...
              {item.address?.map_url && (
                <div className='pt-2'>
                  ...
                  >
                    <MapPin className='h-4 w-4' />
                    <span>{t('view_map') || 'Xem bản đồ'}</span>
                  </a>
                </div>
              )}
            </div>
