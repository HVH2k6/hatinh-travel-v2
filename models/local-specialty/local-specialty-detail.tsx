...
interface LocalSpecialtyDetailViewProps {
  item: LocalSpecialty;
}

export function LocalSpecialtyDetailView({ item }: LocalSpecialtyDetailViewProps) {
    ...
    const syncRealViewCount = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_URL}/local-specialties/${cleanSlug}`,
          { cache: 'no-store' },
        );
        ...
        console.error('Lỗi sync view count đặc sản:', error);
      }
    };
  ...
  React.useEffect(() => {
    ...
    const incrementView = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_URL}/local-specialties/${cleanId}/view`,
          {
            method: 'POST',
            headers: {
        ...
        if (res.ok) {
          ...
          if (data.success && isMounted) {
            setViewCount(data.current_views ?? data.view_count);
          }
        } else {
  ...
  const formatPrice = (price: number) => {
    if (!price || price === 0) return t('contact_price');
    return formatCurrency(price, currentLocale);
  };
  ...
  return (
    <div className='min-h-screen bg-slate-50/50 pb-16 font-sans'>
      ...
      <div className='relative h-[40vh] md:h-[55vh] w-full bg-slate-950'>
        <img
          src={item.image || 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=1200'}
          alt={item.name}
      ...
      <div className='container mx-auto px-4 lg:px-8 mt-8'>
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 items-start'>
          ...
          <div className='lg:col-span-2 space-y-8'>
            ...
            <ReviewSection type='local-specialty' id={item.id} />
          </div>
                ...
                <div className='flex items-start gap-3.5'>
                  ...
                  <div className='space-y-0.5'>
                    ...
                    <p className='text-sm font-semibold text-slate-800'>
                      {item.address?.address_detail ? `${item.address.address_detail}, ` : ''}
                      {item.address?.ward || 'Hà Tĩnh'}
                    </p>
                  </div>
                </div>
              </div>
