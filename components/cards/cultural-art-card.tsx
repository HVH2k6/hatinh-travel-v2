...
export function CulturalArtCard({ item }: Prop) {
  // Use a generic or specific translation depending on what exists, let's assume "card.cultural_art" or we fallback to "explore"
  const t = useTranslations("card.local_specialty") // using local_specialty for fallback to "explore" button text since they probably didn't create translation for cultural_art yet
  
  const fallbackImage = "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=500&auto=format&fit=crop"
  
  return (
    <Link href={`/cultural-arts/${item.slug}`} className="block h-full group">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1 text-xs font-medium text-slate-400">
              ...
              <span className="line-clamp-1">
                {item.address?.address_detail ? `${item.address.address_detail}, ` : ''}{item.address?.ward || ''}
              </span>
            </div>
          </div>
